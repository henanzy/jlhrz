	 ...
	 $.ajax({
			...
			success : function(data) {
				if(data.list.length>0){
					hrz=data.list[0].hrz;
					kxs=data.list[0].Kxs;
					grmj=data.list[0].shijimj;
				}
				
			}
		});
	 $.ajax({
			url:getRootPath()+"/OpcCon/ryjk.action",
			async:false,
			dataType:"json",
			data:{	
				"hrz":"吉利."+selHrz+".读数据.",
	 ...
	 var ygrl=(kxs*(18-wd)*grmj/25*24).toFixed(2);
		...
		function scrollHandlexincreate() {
		 ...
		 function tableToExcel(){
		        ...
		        //列标题
		    	let str = '<tr><th>换热站</th><th>用电量</th><th>用水量</th><th>用热总流量</th>'+
		    	'<th>用热总热量</th>';
		        str+='</tr>'
		...
		var xinwordList = [];
		var data=[{"hrz":hrz,"kxs":kxs,"grmj":grmj,"ssrl":ssrl,"ygrl":ygrl}];
